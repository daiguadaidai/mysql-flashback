...
func (this *Table) initInsertTemplate() {
	template := "/* crc32:%s */ INSERT INTO `%s`.`%s`(`%s`) VALUES(%s);\n"
	this.InsertTemplate = fmt.Sprintf(template, "%d", this.SchemaName, this.TableName,
...
func (this *Table) initUpdateTemplate() {
	template := "/* crc32:%s */ UPDATE `%s`.`%s` SET %s WHERE %s;\n"
	this.UpdateTemplate = fmt.Sprintf(template, "%d", this.SchemaName, this.TableName,
...
func (this *Table) initDeleteTemplate() {
	template := "/* crc32:%s */ DELETE FROM `%s`.`%s` WHERE %s;\n"
	this.DeleteTemplate = fmt.Sprintf(template, "%d", this.SchemaName, this.TableName,
		utils.SqlExprPlaceholderByColumns(this.PKColumnNames, "=", "%s", " AND "))
}
